...
class ClientLocalDataSource {
  ...
  void revertPaidFully({required String entityId}) {
    ...
    entity.timeline.removeWhere((e) {
      ...
      // Remove Paid markers for baseDay and any later dates generated by a
      // previous "Paid fully" action (so revert clears from baseDay onward).
  ...
  void markPaidFully({
    ...
    // Instead of combining remaining scheduled payments into a single aggregate,
    // mark each unpaid scheduled payment from baseDay onward as Paid individually.
    // Mark unpaid scheduled payments from baseDay onward as Paid individually.
    ...
    // If there are no remaining payments, still add a Paid fully marker to
    ...
    // For each remaining scheduled payment, add a statusChange event marking it Paid.
    // Use the payment's own date as the createdAt so payment-scoped status
    // lookups (which match by day) correctly detect the Paid state.
    ...
    // Add a Paid fully marker for the baseDay to indicate the bulk action.
    entity.timeline.add(
      ClientTimelineEvent.statusChange(
        id: _nextId(),
        status: 'Paid fully',
